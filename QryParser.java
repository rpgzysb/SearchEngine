...
public class QryParser {
    ...
    private static Qry createOperator(String operatorName) {
        ...
        // determine whether this operator has parameters
        boolean hasParameters = operatorNameLowerCase.contains("/");
        // distance for proximity operator
        int dist = 0;
        String operatorNameSplit = operatorNameLowerCase;
        if (hasParameters) {
            String[] opSplits = operatorNameLowerCase.split("/");
            operatorNameSplit = opSplits[0];
            // get the distance value
            dist = Integer.parseInt(opSplits[1]);
        }

        // Create the query operator.

        switch (operatorNameSplit) {
        case "#or":
        ...
        case "#near":
            operator = new QryIopNear(dist);
            break;

        case "#window":
            operator = new QryIopWindow(dist);
            break;

        default: {
            syntaxError("Unknown query operator " + operatorNameSplit);
        }
        }
    ...
    private static Qry parseString(String queryString) throws IOException, IllegalArgumentException {
        ...
        while (queryString.length() > 0) {
            ...
            Qry[] qargs = null;
            Double weight = 0.0;
            boolean isWeightOperator = queryTree instanceof QrySopWeight;
            PopData<String, String> p;

            if (queryString.charAt(0) != '#') {
                p = popTerm(queryString);
                String curr = p.getPopped();
                if (isWeightOperator && isDouble(curr)) {
                    weight = Double.parseDouble(curr);
                    // go to the term argument
            ...
            for (int i = 0; i < qargs.length; i++) {
                ...
                // STUDENTS WILL NEED TO ADJUST THIS BLOCK TO HANDLE WEIGHTS IN
                // HW2
                if (isWeightOperator) {
                    ((QrySopWeight)queryTree).appendWeights(weight);
                }
